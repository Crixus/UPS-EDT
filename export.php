<?php
						...
						for($i = 0 ; $i < $nombresCours ; $i++){
							if(isset($coursDuJours[$i])){
								...
								if($Cours->commence_a_heure("$heure:$quart:00")){
									...
									$this->Cell(5*$nbQuartsCours, 5, $Cours->getNomUE(), 1, 0, 'C');
									$boolCours = true;
	...
	$cours_semaine = EmploiDuTemps::cours_semaine($_SESSION['Type'], $_SESSION['Utilisateur']->getId(), EmploiDuTemps::timestamp_debut_semaine($_GET['semaine']));
	
	$pdf->BasicTable($cours_semaine);
